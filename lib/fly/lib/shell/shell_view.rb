class ShellView
  ...
  #
  def initialize(c_l_subclass_inst)
    @c_l_subclass_inst = c_l_subclass_inst
  end
  
  def ask_which_aircrafttype_to_buy
  end
  
  def say_transaction_success
  end
  
  def say_account_balance
  end
  
  def say_transaction_failure
  end
  
  # Looks for method calls that begin with "say_" and hands them off to the CL<subclass> class object
  def method_missing(name,*args)
    if name.to_s.slice(0,4) == "say_"
      @c_l_subclass_inst.send(name,*args)
    end
  end
