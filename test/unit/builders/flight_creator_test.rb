...
class FlightCreatorTest < ActiveSupport::TestCase
  ...
  def test_basics
    ...
    assert_equal(@new_flight.maintenance_duration, 80)
    assert_equal(@new_flight.inflight_duration.to_int,37100)
    assert_equal(@new_flight.dep_airport,@lax_m)
    assert_equal(@new_flight.arr_airport,@zrh_m)
    ...
    assert_equal(@new_flight2.maintenance_duration,120)
    assert_equal(@new_flight2.inflight_duration.to_int,14296)
    assert_equal(@new_flight2.dep_airport,@lax_m)
    assert_equal(@new_flight2.arr_airport,@iad_m)
