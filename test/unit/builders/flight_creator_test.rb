...
class FlightCreatorTest < ActiveSupport::TestCase
  ...
  def test_basics
    ...
    assert_equal(60,@new_flight.boarding_duration_in_seconds)
    assert_equal(70,@new_flight.taxi_duration_in_seconds)
    assert_equal(37100,@new_flight.inflight_duration_in_seconds.to_int)
    ...
    assert_equal(6,@new_flight.code.length)
    assert_equal("LX",@new_flight.code.slice(0,2))
    assert @new_flight.code.slice(2,4).to_i > 0
    ...
    @code = Code.new("NW0022")
    
    @fc << @boarding << @taxi << @arr_airport_iad << @flight_miles_4000
    @fc << @pax_count_200 << @payload_value_flc_150 << @code
    @new_flight2 = @fc.manufacture
    @new_flight2.save
    assert @new_flight2.id > 0
    
    assert_equal(@new_flight2.boarding_duration_in_seconds,15)
    assert_equal(@new_flight2.taxi_duration_in_seconds,45)
    assert_equal(@new_flight2.inflight_duration_in_seconds.to_int,14296)
    ...
    assert_equal("NW0022",@new_flight2.code)
  end
end
